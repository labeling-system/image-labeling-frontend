...
class Workspace extends Component {
  ...
  async componentDidMount () {
    ...
    try {
      const labelResult = await getMostUsedLabels()
    ...
    if (result === null) {
      ...
      try {
        const imageResult = await getWorkingImage()
        console.log(imageResult)
        if (typeof imageResult.data.error === 'undefined') {
          this.setState({
            idData: imageResult.data.image_id,
      ...
      try {
        const imageResult = await getSpecificImage(result)
