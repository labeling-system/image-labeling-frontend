...
class Edit extends Component {
  ...
  async handleGetAllImages (page) {
    try {
      const result = await getAllImages(page)
  ...
  renderRedirect () {
    if (this.state.redirect) {
      const redirectPage = '/workspace/' + this.state.editId
  ...
  async componentDidMount () {
    try {
      const result = await getAllImages(1)
      this.setState({ images: result.data.images })
  ...
  render () {
    ...
    const items = []
    const max_number = Math.ceil(this.state.count / COUNT_PAGE)
